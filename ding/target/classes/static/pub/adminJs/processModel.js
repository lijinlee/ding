...
$(function () {
    $('#wu-datagrid-jbpm').datagrid({
        title: '系统用户',
        url: '/admin/processModel/getPage',
        rownumbers: true,
        singleSelect: false,
        ...
        columns: [[
            ...
            {field: 'suspended', title: '是否挂起', width: 60,formatter:
                function(val, row){
                    if(val==true){
                        return "<a href='/admin/process/convert-to-model?processDefinitionId="+row.id+ "'>转换model</a>";
                    }else{
                        return "<a href='/admin/process/convert-to-model?processDefinitionId="+row.id+ "'>转换model</a>";
                    }
            ...
            {field: 'dffsds', title: '编辑', width: 40,formatter:
                function(val, row){
